/**
 * KDS — écran cuisine (sprint 2, §A).
 * Cycle par carte entière (pas de gestion par article en v1) :
 *   ENVOYEE_CUISINE (items A_PREPARER) → « Commencer » (EN_COURS)
 *   → « Prêt » (PRET, commande PRETE) → « Reprendre » en cas d'erreur.
 * Seuls les articles déjà envoyés (envoye_le non NULL) apparaissent ; un
 * article annulé après envoi reste sur la carte, barré « ANNULÉ ».
 */
...
export function routesKds(app: FastifyInstance): void {
  const gardeCuisine = app.exigerRole('CUISINE', 'MANAGER', 'PROPRIETAIRE');

  // Grille (plus ancienne → plus récente) + colonne « Prêtes » (10 dernières)
  app.get('/api/kds/commandes', { preHandler: gardeCuisine }, async (): Promise<KdsVue> => {
    const [enCuisine, pretes, params] = await Promise.all([
      db
  ...
  app.post('/api/kds/commandes/:id/commencer', { preHandler: gardeCuisine }, async (req) => {
  ...
  app.post('/api/kds/commandes/:id/pret', { preHandler: gardeCuisine }, async (req) => {
    const { id } = req.params as { id: string };
    await db.transaction(async (tx) => {
      const c = await verrouillerCommande(tx, id);
      if (c.statut !== 'ENVOYEE_CUISINE') {
  ...
  app.post('/api/kds/commandes/:id/reprendre', { preHandler: gardeCuisine }, async (req) => {
    const { id } = req.params as { id: string };
    await db.transaction(async (tx) => {
      const c = await verrouillerCommande(tx, id);
