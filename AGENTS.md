...
- **graphify** (`/Users/macbookpro/.agents/skills/graphify/SKILL.md`) transforme le code et la documentation du projet en graphe de connaissances navigable.
- Pour toute question sur le code, son architecture, les relations entre fichiers ou le contenu du projet, utiliser cette compétence et consulter en priorité `graphify-out/graph.json` lorsqu’il existe.
- Déclencheur explicite : `/graphify`.
- Quand l’utilisateur saisit `/graphify`, suivre le skill Graphify avant toute autre action.
